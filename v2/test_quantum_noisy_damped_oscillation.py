...
def run_quantum_noisy_damped_oscillation_prediction():
    ...
    plt.plot(np.arange(len(generated_waveform_for_eval)), generated_waveform_for_eval, label='Predicted (Quantum)', color='red', linestyle='--')
    plt.title('Ground Truth vs. Predicted Noisy Damped Oscillation (Quantum RWKV)')
    plt.xlabel('Time Step (in test segment)')
    plt.ylabel('Value')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plot_filename = os.path.join(results_dir, "noisy_damped_comparison_quantum.png")
    try:
